...
- **One root error boundary.** The root layout exports expo-router's `ErrorBoundary`, so a render error anywhere shows a recovery screen instead of killing the app. When it catches, the layout has already unmounted, which tears down the stream, the settings sync and the query cache. "Try again" also clears the live market store, because the data on screen at the moment of a crash is its most likely cause, then mounts everything again with the session intact. The raw error text is shown in development only.
...
| No schema validation of stream frames on the phone            | JS thread headroom                                                      | A gateway bug would surface as a rendering error instead of a parse error. The shared contract and the gateway's tests guard against it, and the root error boundary recovers from it |
